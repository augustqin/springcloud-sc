package com.example.activityplatform.controller.pages.back.pay;
...
import com.example.activityplatform.config.alipay.AlipayConfig;
import com.example.activityplatform.config.wxpay.WxConfig;
import com.example.activityplatform.util.device.DeviceUtil;
import okhttp3.*;
...
public class PayController {
    ...
    String unifiedPay(Activityorder activityorder, HttpServletRequest request, @RequestHeader("user-agent") String userAgent, HttpSession session) throws AlipayApiException {
        String payType = activityorder.getPaytype();// 先获取用户选择的付款是

        activityorder = activityorderService.getOrderDetailByOrderId(activityorder.getOrderid());
        ...
        if ("余额支付".equals(payType)) {// 暂时没写
            ...
            if ("支付宝".equals(payType)) {
                ...
                session.setAttribute("out_trade_no", activityorder.getOrderid());
                paramerMap.put("out_trade_no", activityorder.getOrderid());// 商户订单编号
                paramerMap.put("total_amount", activityorder.getAllcost()); // 订单要付款金额
                paramerMap.put("subject", "报名活动付款"); // 订单描述
...
//                        request.setAttribute("allCost", activityorder.getAllcost());
//                        request.setAttribute("orderId", activityorder.getOrderid());
...
//    Map<String, String> wxBXiadan(Activityorder activityorder, HttpServletRequest request) {
//        String subject = "活动付款";
//        Integer allCost = activityorder.getAllcost();
////        try {
...
//                data.put("out_trade_no", activityorder.getOrderid());//订单编号
//                data.put("fee_type", "CNY");//货币类型
//                data.put("nonce_str", WXPayUtil.generateNonceStr());//随机字符串
//                //订单有效时间
//                String youxiaoDate = new SimpleDateFormat("yyyyMMddHHmmss").format(new Date(System.currentTimeMillis() + 1000 * 60 * 29));
